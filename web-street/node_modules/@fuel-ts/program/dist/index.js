...
__export(src_exports, {
  ...
  buildPreConfirmationFunctionResult: () => buildPreConfirmationFunctionResult,
  extractInvocationResult: () => extractInvocationResult,
  getAbisFromAllCalls: () => getAbisFromAllCalls,
  getAllResultLogs: () => getAllResultLogs,
...
var getAllResultLogs = /* @__PURE__ */ __name((opts) => {
  ...
  const { main, otherContractsAbis } = getAbisFromAllCalls(functionScopes);
  return (0, import_account.getAllDecodedLogs)({ receipts, mainAbi: main, externalAbis: otherContractsAbis });
}, "getAllResultLogs");
...
var calculateScriptDataBaseOffset = /* @__PURE__ */ __name((maxInputs) => import_abi_coder.SCRIPT_FIXED_SIZE + (0, import_abi_coder.calculateVmTxMemory)({ maxInputs }), "calculateScriptDataBaseOffset");
...
function callResultToInvocationResult(params) {
  ...
  return decodeCallResult({
    callResult,
    scriptResultDecoder: /* @__PURE__ */ __name((scriptResult) => {
...
var isReturnType = /* @__PURE__ */ __name((type) => type === import_transactions2.ReceiptType.Return || type === import_transactions2.ReceiptType.ReturnData, "isReturnType");
...
var decodeContractCallScriptResult = /* @__PURE__ */ __name((params) => {
...
var buildPreConfirmationFunctionResult = /* @__PURE__ */ __name(async (params) => {
  ...
  if (receipts) {
    ...
    gasUsed = (0, import_account3.getGasUsedFromReceipts)(receipts);
  }
  const submitResult = {
    isMultiCall,
    functionScopes,
...
var BaseInvocationScope = class {
  ...
  async fundWithRequiredCoins({
    ...
    request = (0, import_ramda.clone)(request);
    let { feePayerAccount, accountCoinQuantities, ...restAssembleTxParams } = this.assembleTxParameters ?? {};
    request.maxFee = (0, import_math2.bn)(0);
    request.gasLimit = (0, import_math2.bn)(0);
    const provider = this.getProvider();
    const account = this.program.account ?? import_account4.Wallet.generate({ provider });
    const baseAssetId = await provider.getBaseAssetId();
    if (!feePayerAccount) {
      feePayerAccount = account;
    }
    if (!accountCoinQuantities) {
      const outputQuantities = request.outputs.filter((o) => o.type === import_transactions3.OutputType.Coin).map(({ amount, assetId }) => ({ assetId: String(assetId), amount: (0, import_math2.bn)(amount) }));
      accountCoinQuantities = (0, import_account4.mergeQuantities)(outputQuantities, this.requiredCoins);
      if (!accountCoinQuantities.length) {
        accountCoinQuantities.push({ assetId: baseAssetId, amount: (0, import_math2.bn)(0) });
      }
    }
    const assembleTx = /* @__PURE__ */ __name(async () => {
      ...
      await (0, import_account4.setAndValidateGasAndFeeForAssembledTx)({
        gasPrice,
        provider,
    ...
    try {
      ...
      const shouldRetry = await (0, import_account4.consolidateCoinsIfRequired)({
        error,
        account,
        skipAutoConsolidation
      });
  ...
  addContracts(contracts) {
    contracts.forEach((contract) => {
      if (typeof contract === "string") {
        this.transactionRequest.addContractInputAndOutput(new import_address.Address(contract));
      } else {
  ...
  async get() {
    ...
    request.maxFee = (0, import_math2.bn)(0);
    request.gasLimit = (0, import_math2.bn)(0);
    request.inputs = request.inputs.filter((i) => i.type !== import_transactions3.InputType.Coin);
    const provider = this.getProvider();
    const account = this.program.account ?? import_account4.Wallet.generate({ provider });
    const baseAssetId = await provider.getBaseAssetId();
    const allQuantities = request.outputs.filter((o) => o.type === import_transactions3.OutputType.Coin).map(({ amount, assetId }) => ({ assetId: String(assetId), amount: (0, import_math2.bn)(amount) })).concat(this.requiredCoins);
    ...
    const amountForFee = (0, import_math2.bn)("1000000000000000");
