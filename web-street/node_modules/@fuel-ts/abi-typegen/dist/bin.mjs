...
function renderMainTemplate3(params) {
  ...
  const func = abi.functions.find((f) => f.name === "main");
  if (!func) {
    throw new FuelError6(ErrorCode7.ABI_MAIN_METHOD_MISSING, `ABI doesn't have a 'main()' method.`);
  }
  const { enums } = formatEnums({ types });
  const { structs } = formatStructs({ types });
  const { imports } = formatImports({
...
function assembleScripts(params) {
  ...
  files.push(indexFile);
  if (usesCommonTypes) {
    const commonsFilepath = join3(outputDir, "common.ts");
    const file = {
      path: commonsFilepath,
      contents: renderCommonTemplate({ versions })
    };
...
import { ErrorCode as ErrorCode8, FuelError as FuelError7 } from "@fuel-ts/errors";
    ...
    switch (programType) {
      ...
      default:
        throw new FuelError8(
          ErrorCode9.INVALID_INPUT_PARAMETERS,
...
function runTypegen(params) {
  ...
  if (!inputFilepaths?.length && inputs?.length) {
    ...
    throw new FuelError9(
      ErrorCode10.MISSING_REQUIRED_PARAMETER,
  ...
  if (!abiFiles.length) {
    throw new FuelError9(ErrorCode10.NO_ABIS_FOUND, `no ABI found at '${inputs}'`);
  }
