export declare const BURNER_WALLET_ICON = "data:image/svg+xml;utf8;base64,";
export declare const HAS_WINDOW: boolean;
export declare const BURNER_WALLET_PRIVATE_KEY = "burner-wallet-private-key";
export declare const BURNER_WALLET_STATUS = "burner-wallet-status";
export declare const BURNER_SESSION_DETECTED_MESSAGE = "We detected a previous Burner Wallet session. Do you want to continue using the same wallet?";
